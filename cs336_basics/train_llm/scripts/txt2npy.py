...
TS_TRAIN_TXT_PATH = ASSIGNMENT_REPO / "cs336_basics/train_llm/scripts/tinystory_train_chunk_token_counts.txt"
TS_VALID_TXT_PATH = ASSIGNMENT_REPO / "cs336_basics/train_llm/scripts/tinystory_valid_chunk_token_counts.txt"

TS_TRAIN_NPY_PATH = ASSIGNMENT_REPO / "cs336_basics/train_llm/scripts/tinystory_train_chunk_token_counts.npy"
TS_VALID_NPY_PATH = ASSIGNMENT_REPO / "cs336_basics/train_llm/scripts/tinystory_valid_chunk_token_counts.npy"
...
# with open(TS_TRAIN_TXT_PATH, "r") as f:
#     for line in f:
#         m = re.search(r":\s*([\d,]+)\s*tokens", line)
#         if m:
#             counts.append(int(m.group(1).replace(",", "")))
...
# counts = np.array(counts, dtype=np.int64)
# np.save(TS_TRAIN_NPY_PATH, counts)


with open(TS_VALID_TXT_PATH, "r") as f:
    for line in f:
        m = re.search(r":\s*([\d,]+)\s*tokens", line)
        if m:
            counts.append(int(m.group(1).replace(",", "")))
...
counts = np.array(counts, dtype=np.int64)
np.save(TS_VALID_NPY_PATH, counts)